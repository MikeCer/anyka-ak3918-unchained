...
Trigger the update using the camera's update sequence. On the reference camera, this is done by clicking and holding the **reset button** twice until the camera speaker plays a voice prompt (e.g., "Updating" or a similar sound). This triggers `/usr/sbin/update.sh`, which automatically maps the `usr.sqsh4` file from the SD card to MTD2 (`/usr` partition).
